...
パスキー認証の完了時、サーバーはカスタムトークンを発行する前にパスキー所有者が Firebase Auth 上に実在し有効であることを確認する（存在しない uid のカスタムトークンでサインインすると Firebase Auth にユーザーが新規作成されるため）。Firebase コンソールでユーザーを削除すると、そのユーザーのパスキーでのログインは拒否され、次回ログイン試行時に passkey DB からも削除される。ユーザーを無効化した場合もログインは拒否されるが、再有効化に備えてパスキーは残す。
...
1. [MaxMind 無料アカウント](https://www.maxmind.com/en/geolite2/signup) を作成（手動 DL のみなら License Key の発行は不要）
2. アカウントポータルにログインして `GeoLite2-City.mmdb`（tar.gz の中の `.mmdb` 単体）を DL
3. 配置:
...
> **Note:** GeoLite2 は CC BY-SA 4.0 で配布されており、ダウンロードには MaxMind 公式での無料アカウント作成が必要（ポータル DL ページでアカウント認証を求められるため）。手動で DL する分には License Key は不要。`geoipupdate` 等で自動更新を組む場合のみ License Key の発行が必要になる。
>
