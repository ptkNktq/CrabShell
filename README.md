...
| 変数 | デフォルト | 説明 |
|-----|----------|------|
| `FIREBASE_SERVICE_ACCOUNT_PATH` | `firebase-service-account.json` | Firebase サービスアカウントキー |
| `WEBAUTHN_RP_ID` | `localhost` | Relying Party ID（本番はドメイン名） |
| `WEBAUTHN_ORIGIN` | `http://localhost:8080` | 許可するオリジン（カンマ区切りで複数指定可） |
| `PASSKEY_DB_PATH` | `data/passkey.db` | SQLite ファイルパス |
