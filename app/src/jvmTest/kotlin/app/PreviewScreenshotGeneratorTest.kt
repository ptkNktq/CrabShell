package app

import androidx.compose.foundation.layout.Column
import androidx.compose.foundation.layout.Spacer
import androidx.compose.foundation.layout.fillMaxHeight
import androidx.compose.foundation.layout.height
import androidx.compose.foundation.layout.padding
import androidx.compose.foundation.layout.width
import androidx.compose.material3.MaterialTheme
import androidx.compose.material3.Surface
import androidx.compose.ui.Modifier
import androidx.compose.ui.unit.dp
import app.components.DrawerContent
import app.components.NavigationContent
import app.components.Sidebar
import core.previewscreenshot.PreviewScreenshotRecorder
import core.ui.theme.AppTheme
import kotlin.test.Test

/**
 * Sidebar / DrawerContent は app.Screen に依存する app 固有のナビゲーションコンポーネントで、
 * 循環依存になるため feature モジュールの Content からは参照できない。「画面単位」の
 * スクリーンショット対象からは外れるが、ナビゲーションの見た目を確認する価値があるため
 * 例外的にここで生成する。手動実行専用（previewScreenshotTest タスク）で、通常の jvmTest/CI には含めない。
 */
class PreviewScreenshotGeneratorTest {
    private val recorder = PreviewScreenshotRecorder(System.getProperty("previewScreenshot.module") ?: "app")

    @Test
    fun generateSidebarScreenshots() {
        // 折りたたみ状態（デフォルト）: 実際の Sidebar をそのまま描画する
        recorder.save("sidebar_collapsed.png", width = 100, height = 700) {
            AppTheme {
                Sidebar(
                    currentScreen = Screen.Dashboard,
                    onNavigate = {},
                    onSignOut = {},
                    version = "abc1234",
                    isAdmin = true,
                )
            }
        }

        // 展開状態: expanded は Sidebar 内部の remember state で、クリック操作をシミュレートしないと
        // 到達できないため、Sidebar と同じ構造を NavigationContent(expanded = true) で組み立てる
        recorder.save("sidebar_expanded.png", width = 260, height = 700) {
            AppTheme {
                Surface(
                    modifier = Modifier.fillMaxHeight().width(240.dp),
                    color = MaterialTheme.colorScheme.surface,
                    tonalElevation = 2.dp,
                ) {
                    Column(modifier = Modifier.fillMaxHeight().padding(vertical = 8.dp)) {
                        Spacer(modifier = Modifier.height(56.dp))
                        NavigationContent(
                            currentScreen = Screen.Dashboard,
                            onNavigate = {},
                            onSignOut = {},
                            version = "abc1234",
                            isAdmin = true,
                            expanded = true,
                        )
                    }
                }
            }
        }
    }

    @Test
    fun generateDrawerContentScreenshots() {
        // DrawerContent は compact 幅の画面でハンバーガーメニューから開く固定幅ドロワー
        recorder.save("drawercontent_default.png", width = 280, height = 700) {
            AppTheme {
                DrawerContent(
                    currentScreen = Screen.Dashboard,
                    onNavigate = {},
                    onSignOut = {},
                    version = "abc1234",
                    isAdmin = true,
                )
            }
        }
    }
}
