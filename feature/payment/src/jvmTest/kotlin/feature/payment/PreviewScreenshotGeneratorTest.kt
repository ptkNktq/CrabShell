...
class PreviewScreenshotGeneratorTest {
    ...
    private val recorder = PreviewScreenshotRecorder(System.getProperty("previewScreenshot.module") ?: "payment")

    @Test
    fun generatePaymentScreenshots() {
        val users =
            listOf(
        ...
        val monthlyMoney =
            MonthlyMoney(
                ...
                payments =
                    listOf(
                        Payment(id = "pay1", uid = "user1", amount = 2000, paidAt = "2026-07-10T00:00:00Z"),
                    ),
            )

        standardSizePatterns.forEach { pattern ->
            recorder.save(
                fileName = "payment_${pattern.label}.png",
                width = pattern.width,
